...
--
-- Las tablas llevan el prefijo ethobox_ porque este proyecto de Supabase
-- se comparte con CaninaMente Campus (campus_caniciencia), que ya usa los
-- nombres sin prefijo (profiles, courses, course_editions...). Son dos
-- apps con datos independientes que conviven en el mismo proyecto.
...
create table if not exists public.ethobox_profiles (
...
alter table public.ethobox_profiles enable row level security;

drop policy if exists "ethobox_profiles: select propio" on public.ethobox_profiles;
create policy "ethobox_profiles: select propio" on public.ethobox_profiles
...
create table if not exists public.ethobox_cases (
  id uuid primary key default gen_random_uuid(),
  professional_id uuid not null references public.ethobox_profiles(id) on delete cascade,
...
alter table public.ethobox_cases enable row level security;
...
create index if not exists ethobox_cases_professional_id_idx
  on public.ethobox_cases(professional_id);
...
create table if not exists public.ethobox_formulations (
  id uuid primary key default gen_random_uuid(),
  case_id uuid not null references public.ethobox_cases(id) on delete cascade,
  round_number int not null,
  previous_formulation_id uuid references public.ethobox_formulations(id),
...
alter table public.ethobox_formulations enable row level security;

create index if not exists ethobox_formulations_case_id_idx
  on public.ethobox_formulations(case_id);
...
create table if not exists public.ethobox_anamnesis_entries (
  id uuid primary key default gen_random_uuid(),
  formulation_id uuid not null references public.ethobox_formulations(id) on delete cascade,
  case_id uuid not null references public.ethobox_cases(id) on delete cascade,
  raw_text text not null,
  created_at timestamptz not null default now()
);

alter table public.ethobox_anamnesis_entries enable row level security;

create index if not exists ethobox_anamnesis_entries_formulation_id_idx
  on public.ethobox_anamnesis_entries(formulation_id);
